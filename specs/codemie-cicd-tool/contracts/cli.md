...
Source: `specs/codemie-cicd-tool.md` v27, FR-001–006, FR-009,
...
Lint performs no network access. For the complete discovered repository
closure, it parses bounded YAML, validates the exact closed declaration schema,
computes effective project, applies semantic rules, builds the repository
symbol table, and validates natural and Workflow-local references. It never
inserts server defaults. Omitted optional-null fields are valid; omitted/null
authoring-required fields are `E_SCHEMA`, exit 2, before network access.

Only after the complete repository closure passes parsing, schema, semantic,
and reference validation does lint evaluate suspected-plaintext-secret and
deprecated-value warning conditions. It evaluates and emits warnings only for
the declaration selected by `--file`; warning conditions in other declarations
discovered for closure do not produce records. Multiple target warnings are
emitted in bytewise ascending fixed `warningCode` order, then canonical
...
never enter the success object. If any declaration in the repository closure
fails validation, lint emits no warnings: stdout is empty and stderr contains
exactly the failure diagnostic required by the selected output mode.
...
|---|---|---|
| 0 | success | `valid` with zero or more target-declaration warnings; `created`; `updated` |
...
Warnings are emitted only by successful lint after complete repository-closure
validation. On every failure, stdout is empty and no warning is emitted. Text
mode emits exactly one fixed safe diagnostic line to stderr; JSON mode emits
exactly one compact diagnostic object conforming to
...
**Warning ordering**: After complete repository-closure validation succeeds,
warnings for the `--file` declaration are sorted by bytewise ascending fixed
`warningCode`, then by canonical `source.fieldPath`, before their records are
rendered.
